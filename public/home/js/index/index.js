...
define( function( require, exports, module ) {
	...
	Index.prototype._load = function() {
		...
		$.get('/controller/getNews', function(res) {
			var html = '';
		...
		function rank(i) {
			$.get('/controller/getList?data=' + (Number(i)+1), function(res) {
				var html = '';
				$.each(res, function (index, value) {
	...
	Index.prototype._bind = function() {
		...
		$('.wrap .main-rank').on('click', this.rankALLPlay, function () {
			...
			var addID = 0,
				index = $(this).parents('dl').index() / 2 + 1;
			$.ajax({
			...
			$.get('/controller/getMInfo?id=' + addID, function(res) {
				var info = res[0];
