...
define(function (require, exports, module) {
    ...
    Player.prototype.init = function (data_id) {   // data.src, data.name, data.master
        var self = this;
        $.get('/controller/getMInfo?id=' + data_id, function (res) {
    ...
    Player.prototype._bind = function() {
        ...
        $(this.audio).on('loadstart', function() {     // 正在加载 loading
            ...
            var data_src = self.audio.src;
            $.get('/controller/getMInfo?src=' + data_src, function(res) {
                ...
                $(self.audio).attr('data-id', res[0].music_id);
                $('.play-head a').attr('href', './result?id=' + res[0].music_id);
                $('.play-ing .ptitle a').attr('href', './result?id=' + res[0].music_id);
            });
